...
# Contributions by Aidan Franits, October 2023
...
class Message:
	...
	def upgrade(self, bodydata: bytes):
...
class MetaData(Message):
	...
	def _data(self):
		the_return = '{'
		var_values = vars(self)
		var_list = list(var_values)
		
		for i in range(0, len(var_list)):
			if var_list[i] != "type":
				the_return += "\""
				the_return += var_list[i]
				the_return += "\":"
				string_value = isinstance(var_values[var_list[i]], str)
				if string_value:
					the_return += "\""
					the_return += var_values[var_list[i]]
					the_return += "\""
				else:
					the_return += str(var_values[var_list[i]])
	...
	def _setdata(self, data: bytes):
		...
		for i in range(0, len(data_list)):
			...
			if len(current_data) > 1:
				...
				setattr(self, current_data[0], "")

class MetaData2(MetaData):
	msgtype = 0x2A
	
	def _setdata(self, data: bytes):
		if len(data) > 0 and data[0] < 0x20: #Non-printing character.
			data = data[4:]
		
		if data[len(data)-1] < 0x20:
...
opened_info = [
   ...
   _send_string("/etc/box_name", "MKA"),
   CarPlay(101),
]

startup_info = [
	_send_int("/tmp/screen_dpi", 160),] + [
_send_int("/etc/android_work_mode", 1),
] + [
	Open(),
]
