#include "MKA_Socket.h"

// Create a new socket message.
SocketMessage *SocketCreateMessage(const uint8_t opcode, const uint16_t length)
{
    SocketMessage * message = (SocketMessage*) malloc(sizeof(SocketMessage));

    message->opcode = opcode;
    message->len = length;
    message->data = (uint8_t *) malloc(length*sizeof(uint8_t));

    return message;
}

// Clear a socket message from memory.
void SocketClearMessage(SocketMessage* message)
{
    free(message->data);
    free(message);
}

// Populate a socket message with data bytes.
void SocketFillMessageBytes(SocketMessage* message, uint8_t* data)
{
    memcpy(message->data, data, message->len);
}

// Refresh a socket message.
void SocketRefreshMessage(SocketMessage* message, const uint8_t opcode, const uint16_t length)
{
    message->opcode = opcode;
    message->len = length;

    free(message->data);
    message->data = (uint8_t *) malloc(length * sizeof(uint8_t));
}

// Create a new socket with server and client ports.
MKASocket* SocketCreate()
{
    remove(SOCKET_PATH);

    MKASocket* sock = (MKASocket*) malloc(sizeof(MKASocket));

    sock->server = socket(AF_UNIX, SOCK_STREAM, 0);

    struct sockaddr_un path;
    path.sun_family = AF_UNIX;
    strcpy(path.sun_path, SOCKET_PATH);

    int res = bind(sock->server, (struct sockaddr *) &path, sizeof(path));
    listen(sock->server, 5);

    //the_return->clients = accept(the_return->server_socket, NULL, NULL);

    return sock;
}

//Clear the socket from memory.
void SocketClose(MKASocket* socket)
{
    //TODO: Close the sockets?
    free(socket);
}

//Read binary data from the socket.
ssize_t SocketReadBytes(MKASocket* socket, uint8_t* data, const int length)
{
    return recv(socket->client, data, length, 0);
}

//Write binary data to the socket.
void SocketWriteBytes(MKASocket* socket, uint8_t* data, const int length)
{
    send(socket->client, data, length, 0);
}

// Read a message from the socket.
int SocketReadMessage(MKASocket* socket, SocketMessage* message, const int length) {
    uint8_t read_data[length];
    const int message_size = SocketReadBytes(socket, read_data, length);

    if (message_size <= 0) {
        return message_size;
    }

    if (message_size < sizeof(SOCKET_START) + 2) {
        return -1;
    }

    for(uint8_t i=0;i<sizeof(SOCKET_START)-1;i+=1) {
        if(read_data[i] != (uint8_t)SOCKET_START[i])
            return -1;
    }

    const uint8_t opcode = read_data[sizeof(SOCKET_START) - 1], msg_length = read_data[sizeof(SOCKET_START)]-1;

    SocketRefreshMessage(message, opcode, msg_length);
    for(uint8_t i=0;i<msg_length;i+=1)
        message->data[i] = read_data[sizeof(SOCKET_START) + 1 + i];

    return msg_length;
}

// Write a message to the socket.
void SocketWriteMessage(MKASocket *socket, SocketMessage *message)
{
    if(message->len + 1 > SOCKET_MAX_DATA_SIZE) {
        return;
    }

    uint8_t write_data[message->len + 10];

    for(uint8_t i=0;i<sizeof(SOCKET_START)-1;i+=1)
        write_data[i] = SOCKET_START[i];

    write_data[7] = message->opcode;
    write_data[8] = (uint8_t)(message->len + 1);

    for(int i = 0; i < message->len; i+=1) {
        write_data[i+9] = message->data[i];
    }

    uint8_t checksum = 0;
    for(uint8_t i = 0; i < sizeof(write_data) - 1; i += 1) {
        checksum ^= write_data[i];
    }

    write_data[sizeof(write_data)-1] = checksum;

    SocketWriteBytes(socket, write_data, message->len + SOCKET_FRAME_SIZE);
}

//Write an IBus message to the socket.
void writeIBusToSocket(MKASocket* socket, IBus_Message* message) {
    SocketMessage* sock_message = createSocketMessage(0x68, message->l + 4);

    uint8_t sock_bytes[message->l + 4];
    getBytes(message, sock_bytes);

    fillSocketMessageBytes(sock_message, sock_bytes);

    writeSocketMessage(socket, sock_message);
    clearSocketMessage(sock_message);
}
